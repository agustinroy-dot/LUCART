...
def export_data(type):
    def generate():
        ...
        if type == 'customers':
            ...
            si.truncate(0)
            si.seek(0)
            for r in Customer.query.yield_per(1000):
            ...
            si.truncate(0)
            si.seek(0)
            # Cannot use yield_per directly with 1:N joinedload easily, but since Order to Customer is many-to-one,
            # it might work. Let's stick to eager loading safely.
            for r in Order.query.options(joinedload(Order.customer)).yield_per(1000):
            ...
            si.truncate(0)
            si.seek(0)
            for r in Transaction.query.yield_per(1000):
            ...
            si.truncate(0)
            si.seek(0)
            for r in Material.query.yield_per(1000):
    ...
    return Response(
        ...
        headers={"Content-disposition":
                 f"attachment; filename={filename_map[type]}"}
    )
